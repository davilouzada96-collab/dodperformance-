(() => {
  ...
  function exportBrainSelectionAsJson() {
    if (state.sceneName !== "brain") {
      setInfo("Exportacao disponivel na cena Cerebro 3D.");
      return;
    }
  ...
  function buildRegionSurfaceMesh(region, options = {}) {
    ...
    for (let i = 0; i < attr.count; i += 1) {
      ...
      let fold = 0;
      if (isCortical) {
        const g1 = Math.sin((nx * 8.7) + (ny * 6.4) + (nz * 7.1) + phase);
        const g2 = Math.sin((nx * 16.4) - (ny * 12.2) + (nz * 10.7) + (phase * 0.8));
        const sulcus = Math.abs(Math.sin((nx * 9.1) - (ny * 8.6) + phase));
        fold = ((g1 * 0.08) + (g2 * 0.04) - (sulcus * 0.06)) * foldGain;
      }
      ...
      if (isCortical) {
        ...
        anatomyWarp += (lateral * lateral * 0.05) - (medial * medial * 0.09);

        if (macro === "frontal") {
          anatomyWarp += (anterior * anterior * 0.18) + (superior * 0.06);
          anatomyWarp -= posterior * 0.08;
        } else if (macro === "parietal") {
          anatomyWarp += (superior * superior * 0.14) + (posterior * 0.05);
          anatomyWarp -= inferior * 0.05;
        } else if (macro === "temporal") {
          anatomyWarp += (inferior * inferior * 0.17) + (anterior * 0.08);
          anatomyWarp -= superior * 0.08;
        } else if (macro === "occipital") {
          anatomyWarp += (posterior * posterior * 0.2) + (superior * 0.04);
          anatomyWarp -= anterior * 0.1;
        } else if (macro === "insula") {
          anatomyWarp += (inferior * 0.05) + (medial * 0.03);
    ...
    const material = new THREE.MeshStandardMaterial({
      color: new THREE.Color(region.color || "#7aa7ff"),
      transparent: true,
      opacity: isCortical ? 0.35 : 0.46,
      roughness: 0.76,
      metalness: 0.05,
      emissive: new THREE.Color(region.color || "#7aa7ff"),
      emissiveIntensity: isCortical ? 0.06 : 0.09,
    });
  ...
  function applyBrainFilters() {
    if (state.sceneName !== "brain") return;
    ...
    if (state.brainSelections.length === 0) {
      ...
      setInfo(`Filtro ativo: ${visibleCount} regioes visiveis no cerebro 3D.`);
    }
  }
  ...
  function buildBrainScene(data) {
    ...
    cortexRegions.forEach((region) => {
      ...
      shell.renderOrder = 1;
      const points = buildNodePoints(region.nodes, region.color, 0.055, 0.86);
      points.userData = {
        region,
        baseSize: 0.055,
        baseOpacity: 0.86,
    ...
    diencephalonRegions.forEach((region) => {
      ...
      shell.userData.baseEmissiveIntensity = shell.material.emissiveIntensity;
      shell.renderOrder = 1;
      const points = buildNodePoints(region.nodes, region.color, 0.075, 0.95);
      points.userData = {
        region,
        baseSize: 0.075,
        baseOpacity: 0.95,
    ...
    applyBrainFilters();
    setInfo("Cerebro 3D anatomico ativo. Use filtro/busca e clique em uma regiao.");
  }
  ...
  canvas.addEventListener("click", (event) => {
    if (state.sceneName !== "brain" || state.brainPickables.length === 0) return;
