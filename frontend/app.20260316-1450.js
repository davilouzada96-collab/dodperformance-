(() => {
  ...
  const state = {
    sceneName: params.get("scene") || "sphere",
  ...
  if (!allowedScenes.has(state.sceneName)) state.sceneName = "sphere";
  ...
  const labelRenderer = THREE.CSS2DRenderer ? new THREE.CSS2DRenderer() : null;
  if (labelRenderer) {
  ...
  function exportBrainSelectionAsJson() {
    if (state.sceneName !== "brain" && state.sceneName !== "brain_map") {
      setInfo("Exportacao disponivel na cena Cerebro realista.");
      return;
    }
  ...
  function createLabel(text, className = "node-label", yOffset = 3) {
  ...
  function buildPulseScene(data) {
    ...
    setInfo("Rede com pulsos ativa. Sinais luminosos percorrem as sinapses.");
    resetStatusXor();
  }
  ...
  async function buildRealisticBrainScene() {
    ...
    const group = new THREE.Group();
    group.add(model);
    const anatomy = generateBrainAnatomyData(1.0);
    const subregionCatalog = anatomy?.meta?.subregion_catalog || {};

    const groupSpacing = {
      frontal: new THREE.Vector3(-30, 10, 10),
      parietal: new THREE.Vector3(0, 25, 0),
      temporal: new THREE.Vector3(20, 0, 20),
      occipital: new THREE.Vector3(30, 10, -10),
      talamo: new THREE.Vector3(0, 0, 0),
    };
    ...
    const overlayNodes = [
      { id: "frontal", name: "Frontal", group: "frontal", size: 3.6, major: true },
      { id: "parietal", name: "Parietal", group: "parietal", size: 3.4, major: true },
      { id: "temporal", name: "Temporal", group: "temporal", size: 3.8, major: true },
      { id: "occipital", name: "Occipital", group: "occipital", size: 3.2, major: true },
      { id: "talamo", name: "Talamo", group: "talamo", size: 4.2, major: true },
      { id: "f1", name: "F1", group: "frontal", size: 0.8 },
      { id: "f2", name: "F2", group: "frontal", size: 0.8 },
      { id: "f3", name: "F3", group: "frontal", size: 0.8 },
      { id: "p1", name: "P1", group: "parietal", size: 0.8 },
      { id: "p2", name: "P2", group: "parietal", size: 0.8 },
      { id: "t1", name: "T1", group: "temporal", size: 0.8 },
      { id: "t2", name: "T2", group: "temporal", size: 0.8 },
      { id: "t3", name: "T3", group: "temporal", size: 0.8 },
      { id: "o1", name: "O1", group: "occipital", size: 0.8 },
      { id: "o2", name: "O2", group: "occipital", size: 0.8 },
    ];
    ...
    const overlayEdges = [
      ...
      ["talamo", "temporal"],
      ["talamo", "occipital"],
      ["frontal", "f1"], ["frontal", "f2"], ["frontal", "f3"],
      ["parietal", "p1"], ["parietal", "p2"],
      ["temporal", "t1"], ["temporal", "t2"], ["temporal", "t3"],
      ["occipital", "o1"], ["occipital", "o2"],
      ["f1", "p1"], ["f2", "t1"], ["f3", "talamo"],
      ["p2", "o1"], ["t2", "o2"], ["t3", "talamo"], ["p1", "t1"],
    ];

    const regionMetaByGroup = {
      frontal: {
        id: "frontal",
        label: "Lobo frontal",
        kind: "lobe",
        parent: "frontal",
        subregions: subregionCatalog.frontal || [],
        color: "#8cc8ff",
        center: [groupSpacing.frontal.x, groupSpacing.frontal.y, groupSpacing.frontal.z],
      },
      parietal: {
        id: "parietal",
        label: "Lobo parietal",
        kind: "lobe",
        parent: "parietal",
        subregions: subregionCatalog.parietal || [],
        color: "#a8b6ff",
        center: [groupSpacing.parietal.x, groupSpacing.parietal.y, groupSpacing.parietal.z],
      },
      temporal: {
        id: "temporal",
        label: "Lobo temporal",
        kind: "lobe",
        parent: "temporal",
        subregions: subregionCatalog.temporal || [],
        color: "#ffc978",
        center: [groupSpacing.temporal.x, groupSpacing.temporal.y, groupSpacing.temporal.z],
      },
      occipital: {
        id: "occipital",
        ...
        kind: "lobe",
        parent: "occipital",
        subregions: subregionCatalog.occipital || [],
        color: "#ffdd86",
        center: [groupSpacing.occipital.x, groupSpacing.occipital.y, groupSpacing.occipital.z],
      },
      talamo: {
        id: "thalamus",
        label: "Talamo",
        kind: "diencephalon",
        parent: "thalamus",
        subregions: subregionCatalog.thalamus || [],
        color: "#ff9f7a",
        center: [groupSpacing.talamo.x, groupSpacing.talamo.y, groupSpacing.talamo.z],
      },
    };
    ...
    const regionMeshByGroup = new Map();
    ...
    overlayNodes.forEach((node) => {
      const color = groupColors[node.group] || 0x89b5ff;
      const mat = new THREE.MeshStandardMaterial({
        color,
        emissive: color,
        emissiveIntensity: node.major ? 0.26 : 0.18,
        transparent: true,
        opacity: node.major ? 0.96 : 0.82,
        roughness: 0.35,
        metalness: 0.1,
      });
      ...
      const anchor = groupSpacing[node.group].clone();
      const spread = node.major ? 3.5 : 12;
      mesh.position.copy(anchor).add(new THREE.Vector3(
        (Math.random() - 0.5) * spread,
        (Math.random() - 0.5) * spread,
        (Math.random() - 0.5) * spread
      ));
      mesh.userData.velocity = new THREE.Vector3();
      mesh.userData.group = node.group;
      mesh.userData.size = node.size;
      mesh.userData.anchor = anchor;

      const label = createLabel(
        node.name,
        node.major ? "group-label" : "node-label",
        node.major ? (node.size + 2.4) : (node.size + 1.3),
      );
      if (label) mesh.add(label);

      if (node.major) {
        const region = regionMetaByGroup[node.group];
        mesh.userData.region = region;
        mesh.userData.baseOpacity = mat.opacity;
        mesh.userData.baseEmissiveIntensity = mat.emissiveIntensity;
        state.brainEntries.push({ region, points: null, shell: mesh, marker: null });
        state.brainPickables.push(mesh);
        regionMeshByGroup.set(node.group, mesh);
      }
    ...
    const linePositions = new Float32Array(overlayEdges.length * 6);
    ...
    function updateOverlayLines() {
      for (let i = 0; i < overlayEdges.length; i += 1) {
        const [a, b] = overlayEdges[i];
        const na = nodeMap.get(a);
        const nb = nodeMap.get(b);
        if (!na || !nb) continue;
        const idx = i * 6;
        linePositions[idx] = na.position.x;
        linePositions[idx + 1] = na.position.y;
        linePositions[idx + 2] = na.position.z;
        linePositions[idx + 3] = nb.position.x;
        linePositions[idx + 4] = nb.position.y;
        linePositions[idx + 5] = nb.position.z;
      }
      lineAttr.needsUpdate = true;
    }

    function applyOverlayForces() {
      ...
      for (let i = 0; i < meshes.length; i += 1) {
        ...
        for (let j = i + 1; j < meshes.length; j += 1) {
          ...
          a.userData.velocity.add(delta);
          b.userData.velocity.sub(delta);
        }
      }

      overlayEdges.forEach(([idA, idB]) => {
        ...
        const desired = a.userData.group === b.userData.group ? 16 : 34;
        const stretch = dist - desired;
        delta.normalize().multiplyScalar(stretch * spring);
        a.userData.velocity.add(delta);
        b.userData.velocity.sub(delta);
      });

      meshes.forEach((mesh) => {
        const target = groupSpacing[mesh.userData.group];
        const toAnchor = new THREE.Vector3().subVectors(target, mesh.position).multiplyScalar(centering);
        mesh.userData.velocity.add(toAnchor);
        mesh.userData.velocity.multiplyScalar(damping);
        mesh.position.add(mesh.userData.velocity);
        mesh.position.lerp(target, 0.02);
      });

      regionMeshByGroup.forEach((mesh, groupId) => {
        const region = regionMetaByGroup[groupId];
        if (!region) return;
        region.center = [mesh.position.x, mesh.position.y, mesh.position.z];
      });
    }
    ...
    if (brainKindFilter) brainKindFilter.disabled = true;
    if (brainSearchInput) brainSearchInput.disabled = true;
    ...
    statusPredictions.textContent = [
      ...
      "overlay: lobos + talamo com labels",
      "comparar A/B: clique ou Shift+clique",
    ].join("\n");
    ...
    setInfo("Cerebro realista ativo com sobreposicao dos lobos no espaco.");
  }
  ...
  canvas.addEventListener("click", (event) => {
    if ((state.sceneName !== "brain" && state.sceneName !== "brain_map") || state.brainPickables.length === 0) return;
